...
import top.kthirty.core.tool.utils.*;
...
/**
 * 常用工具方法集合，仅调用其他方法，不实现具体逻辑，方便重构或替换工具类
 *
 * @author Kthirty
 * @since Created in 2023/11/19
 */
public class Func {
    public static boolean equalsAny(final CharSequence string, final CharSequence... searchStrings) {
        return StrUtil.equalsAny(string, searchStrings);
    }

    public static String upperCase(final String s) {
        return isBlank(s) ? StringPool.EMPTY : s.toUpperCase();
    }
     ...
     * 注意：不支持链式Bean，链式用 copyProperties
     *
     * @param source 源对象，为null时直接返回null
     * @param clazz  类名
     * @param <T>    泛型标记
     * @return T
     */
    public static <T> T copyIgnoreNull(Object source, Class<T> clazz) {
        return source == null ? null : copy(source, clazz);
    }
     ...
     * @param func      待执行func
     */
    public static void doIf(boolean condition, VoidFunc0 func) {
        if (condition) {
